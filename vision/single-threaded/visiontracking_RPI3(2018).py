...
# Reduce the size of video to 400x300 (3:4) so CPU can process faster (real resolution is actually 175x143).
cap.set(3,200)
cap.set(4,300)
...
#cv2.namesWindow('edged')
#Create tracking window and make it fullscreen for image streaming.
...
cv2.createTrackbar('hmin', 'HueComp',23,179,nothing)
cv2.createTrackbar('hmax', 'HueComp',36,179,nothing)
cv2.createTrackbar('smin', 'SatComp',174,255,nothing)
cv2.createTrackbar('smax', 'SatComp',255,255,nothing)
cv2.createTrackbar('vmin', 'ValComp',46,255,nothing)
cv2.createTrackbar('vmax', 'ValComp',147,255,nothing)
...
while(1):
    ...
    if isFreeMode.value == False:
        cap.set(cv2.CAP_PROP_BRIGHTNESS, 0.05)
...
###############################################################################    
    ...
    if isFreeMode.value == True:
        ...
        if str(hmnValueFree) != str(hmnFree):
            hmnWriteFree = open('/home/pi/Desktop/Values/FreeMode/hmn.txt', 'w')
            test = str(hmnFree)
            hmnWriteFree.write(test)
        if str(hmxValueFree) != str(hmxFree):
            hmxWriteFree = open('/home/pi/Desktop/Values/FreeMode/hmx.txt', 'w')
            test = str(hmxFree)
            hmxWriteFree.write(test)
        if str(smnValueFree) != str(smnFree):
            smnWriteFree = open('/home/pi/Desktop/Values/FreeMode/smn.txt', 'w')
            test = str(smnFree)
            smnWriteFree.write(test)
        if str(smxValueFree) != str(smxFree):
            smxWriteFree = open('/home/pi/Desktop/Values/FreeMode/smx.txt', 'w')
            test = str(smxFree)
            smxWriteFree.write(test)
        if str(vmnValueFree) != str(vmnFree):
            vmnWriteFree = open('/home/pi/Desktop/Values/FreeMode/vmn.txt', 'w')
            test = str(vmnFree)
            vmnWriteFree.write(test)
        if str(vmxValueFree) != str(vmxFree):
            vmxWriteFree = open('/home/pi/Desktop/Values/FreeMode/vmx.txt', 'w')
            test = str(vmxFree)
    ...
    if isFreeMode.value == False:
        ...
        if str(hmnValueTape) != str(hmnTape): 
            hmnWriteTape = open('/home/pi/Desktop/Values/TapeMode/hmn.txt', 'w')
            test = str(hmnTape)
            hmnWriteTape.write(test)
        if str(hmxValueTape) != str(hmxTape):
            hmxWriteTape = open('/home/pi/Desktop/Values/TapeMode/hmx.txt', 'w')
            test = str(hmxTape)
            hmxWriteTape.write(test)
        if str(smnValueTape) != str(smnTape):
            smnWriteTape = open('/home/pi/Desktop/Values/TapeMode/smn.txt', 'w')
            test = str(smnTape)
            smnWriteTape.write(test)
        if str(smxValueTape) != str(smxTape):
            smxWriteTape = open('/home/pi/Desktop/Values/TapeMode/smx.txt', 'w')
            test = str(smxTape)
            smxWriteTape.write(test)
        if str(vmnValueTape) != str(vmnTape):
            vmnWriteTape = open('/home/pi/Desktop/Values/TapeMode/vmn.txt', 'w')
            test = str(vmnTape)
            vmnWriteTape.write(test)
        if str(vmxValueTape) != str(vmxTape):
            vmxWriteTape = open('/home/pi/Desktop/Values/TapeMode/vmx.txt', 'w')
            test = str(vmxTape)
...
###############################################################################
    ...
    if point1x.value > 93 and point3x.value < 82:
        switcher = 0

    #If the rectangle is rotated set switcher as a toggle for rotated calculations.
    if point1x.value < 82 and point3x.value > 93:
        switcher = 1
...
###############################################################################
    ...
    hmn = int(NWTBTrackbarValueHmn.value) #hmn = cv2.getTrackbarPos('hmin','HueComp')	#REPLACE WITH NETWORKTABLE VALUES
    hmx = int(NWTBTrackbarValueHmx.value) #hmx = cv2.getTrackbarPos('hmax','HueComp')	#REPLACE WITH NETWORKTABLE VALUES
    smn = int(NWTBTrackbarValueSmn.value) #smn = cv2.getTrackbarPos('smin','SatComp')	#REPLACE WITH NETWORKTABLE VALUES
    smx = int(NWTBTrackbarValueSmx.value) #smx = cv2.getTrackbarPos('smax','SatComp')	#REPLACE WITH NETWORKTABLE VALUES
    vmn = int(NWTBTrackbarValueVmn.value) #vmn = cv2.getTrackbarPos('vmin','ValComp')	#REPLACE WITH NETWORKTABLE VALUES
    vmx = int(NWTBTrackbarValueVmx.value) #vmx = cv2.getTrackbarPos('vmax','ValComp') #REPLACE WITH NETWORKTABLE VALUES
...
###############################################################################
    ...
    k = cv2.waitKey(5) & 0xFF #Change 5 to a different number to expirement with framerate.
    if k == 27:
        break
